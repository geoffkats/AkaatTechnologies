...
@section('title', 'Edit Product - AKAAT Technologies')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ route('admin.products.show', $product) }}" class="bg-akaat-blue text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors">
                        View Product
                    </a>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="short_description" name="short_description" rows="3"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-akaat-blue focus:border-akaat-blue">{{ old('short_description', $product->short_description) }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="6" required
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-akaat-blue focus:border-akaat-blue">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="dimensions" name="dimensions" value="{{ old('dimensions', $product->dimensions) }}" placeholder="e.g., 10 x 5 x 3 cm"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-akaat-blue focus:border-akaat-blue">
                        @error('dimensions')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    @if($product->images && count($product->images) > 0)
                        @foreach($product->images as $index => $image)
                            <div class="image-input-group mb-4">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Image URL {{ $index + 1 }}</label>
                                <div class="flex gap-2">
                                    <input type="url" name="images[]" value="{{ old('images.' . $index, $image) }}" placeholder="https://example.com/image.jpg"
